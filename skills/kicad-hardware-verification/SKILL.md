...
- `import wx; _APP = wx.App(False)` BEFORE `import pcbnew`, always, or expect
...
3. **Courtyard layer** - conservative clearance truth; overlaps here stall
   autorouters even when bodies physically fit.
...
- CLI batch mode (v2.1-v2.2.4) routes fine but can HANG at exit and never
  write the .ses (scheduler thread busy-loop; output only materializes on
  graceful exit). The reliable headless path is v2.2.4's REST API: start with
  `FREEROUTING__API_SERVER__AUTHENTICATION__ENABLED=false`, submit the DSN as
  a job, poll, download the .ses over HTTP.
- SES files can contain empty or `::N` component names (pin headers, USB
  footprints) that make `ImportSpecctraSES` fail silently; clean with
